...
class DoAuthenticationService(DomainService):
    def __init__(
        ...
        self.__get_user_by_email_and_password_service = get_user_by_email_and_password_service
        super().__init__()

    async def execute(self, request_login: RequestLogin) -> Optional[str]:
        encrypted_password = hashlib.sha256(request_login.password.encode("utf-8")).hexdigest()
        user = await self.__get_user_by_email_and_password_service.execute(request_login.email, encrypted_password)
        if user:
