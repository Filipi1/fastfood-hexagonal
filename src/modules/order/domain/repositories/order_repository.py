...
class OrderRepository(GoRepository[OrderEntity]):
    ...
    async def get_user_current_order(self, user_id: str) -> OrderEntity:
        return await self.find_one({ "user_id": user_id, "status": GoIn([OrderStatus.WAITING_PAYMENT]) })
    ...
    async def abandon_all_active_orders_for_user(self, user_id: str) -> bool:
        return await self.update({ "status": OrderStatus.ABANDONED }, { "user_id": user_id, "status": GoIn([OrderStatus.WAITING_PAYMENT]) })