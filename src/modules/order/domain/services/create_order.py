...
class CreateOrderService(DomainService):
    def __init__(self, get_all_products_by_codes_service: GetAllProductsByCodesService, order_repository: OrderRepository):
        self.get_all_products_by_codes_service = get_all_products_by_codes_service
        self.order_repository = order_repository
    
    async def execute(self, user_id: str, order_request: List[RequestOrder]) -> OrderEntity:
        products = await self.get_all_products_by_codes_service.execute([request.product_code for request in order_request])
        products_order = [OrderProduct.from_product(product, request.quantity) for product, request in zip(products, order_request)]
